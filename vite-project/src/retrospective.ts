...
// Task 1
export function getSubjectsNames (subjects: Record<SubjectsNames, SubjectsDescr>): string{
...
export function getTotalTeachers (subjects: Record<SubjectsNames, SubjectsDescr>): number {
    return Object.values(subjects).reduce((acc , { teachers }) => acc + teachers,
    0)
}

export function getTotalStudents (subjects: Record<SubjectsNames, SubjectsDescr>): number {
...
export function getAverage (total:number, amount: number): number {
